...
const OrderDetail = () => {
  ...
  const fetchOrderDetail = async () => {
    ...
    try {
      ...
      if (err.message === "unauthorized") {
        ...
        setError(
          err.message || "เกิดข้อผิดพลาดในการโหลดข้อมูล กรุณาลองใหม่อีกครั้ง",
        );
      }
    } finally {
      setLoading(false);
 ...
 // -------------------------------------------------------------
  ...
  const handleUploadSlip = async () => {
    ...
    try {
      ...
      const uploadRes = await fetch(`${apiBaseUrl}/payments/${paymentId}/upload-slip`, {
        method: "POST",
        headers: { "Authorization": `Bearer ${token}` }, // ไม่ต้องใส่ Content-Type
        body: formData
      });
      ...
      if (uploadData.success) {
        // ❌ เอาการยิง /orders/:id/status ออกไปแล้ว ปัญหา 403 จะไม่เกิดแน่นอน
        
        alert("ส่งหลักฐานการชำระเงินสำเร็จ! ระบบกำลังตรวจสอบยอดเงิน");
        clearSlip();
        fetchOrderDetail(); // รีเฟรชหน้า เพื่อดึงข้อมูลออเดอร์ (พร้อมสถานะที่อัปเดตใหม่) มาแสดง
      } else {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 pt-20 px-4 text-center">
        ...
        <button
          ...
          className="px-8 py-3 bg-[#79A68F] text-white font-bold rounded-xl hover:bg-[#5E8570] transition shadow-lg shadow-[#79A68F]/30"
        >
  ...
  const formattedDate = new Date(orderInfo.created_at).toLocaleDateString(
    "th-TH",
    {
      year: "numeric",
      month: "long",
  ...
  const canUploadSlip =
    orderInfo.status === "pending" || orderInfo.status === "processing";
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans pt-28 pb-20">
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        {/* ✅ โชว์เหตุผลการยกเลิก (ถ้าออเดอร์ถูกยกเลิก) */}
        {orderInfo.status === "cancelled" && orderInfo.cancel_reason && (
          <div className="bg-rose-50 border border-rose-100 p-6 rounded-3xl flex items-start gap-4 shadow-sm animate-in fade-in zoom-in-95">
            <XCircle className="text-rose-500 shrink-0 mt-0.5" size={28} />
            <div>
              <p className="text-lg font-black text-rose-800 mb-1">
                คำสั่งซื้อถูกยกเลิกแล้ว
              </p>
              <p className="text-sm font-medium text-rose-600 leading-relaxed">
                เหตุผล: {orderInfo.cancel_reason}
              </p>
            </div>
          </div>
        )}
        ...
        {canUploadSlip && (
          <div
            className={`bg-white rounded-3xl p-6 sm:p-8 border-2 shadow-sm relative overflow-hidden transition-colors ${orderInfo.status === "processing" ? "border-blue-200" : "border-amber-200"}`}
          >
            <div
              className={`absolute top-0 left-0 w-2 h-full ${orderInfo.status === "processing" ? "bg-blue-400" : "bg-amber-400"}`}
            ></div>

            <div className="flex items-center gap-3 mb-6">
              <div
                className={`p-3 rounded-xl ${orderInfo.status === "processing" ? "bg-blue-100 text-blue-600" : "bg-amber-100 text-amber-600"}`}
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              ...
              <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100 h-full flex flex-col justify-center">
                ...
                <div className="flex items-center gap-4 mb-4">
                  <img
                    src="https://upload.wikimedia.org/wikipedia/en/thumb/5/58/Kasikornbank_Logo.svg/1200px-Kasikornbank_Logo.svg.png"
                    alt="KBank"
                    className="h-8 object-contain"
                  />
                  <div>
                    <p className="font-bold text-gray-900">ธนาคารกสิกรไทย</p>
                    <p className="text-xl font-black text-[#79A68F] tracking-wider">
                      123-4-56789-0
                    </p>
                  </div>
                </div>
                <p className="text-sm text-gray-600 font-medium">
                  ชื่อบัญชี:{" "}
                  <span className="text-gray-900">บจก. เพ็ทเทอเรน</span>
                </p>
              </div>
              ...
              <div>
                {!displaySlip ? (
                  ...
                  <div className="relative border-2 border-gray-200 rounded-2xl overflow-hidden group">
                    <img
                      src={
                        displaySlip.startsWith("http") ||
                        displaySlip.startsWith("blob:")
                          ? displaySlip
                          : `${apiBaseUrl.replace("/api", "")}${displaySlip}`
                      }
                      alt="Slip Preview"
                    ...
                    <div className="absolute bottom-0 left-0 w-full p-3 bg-white/90 backdrop-blur border-t border-gray-200 flex justify-between items-center">
                      <div className="flex items-center gap-2 truncate pr-4">
                        <ImageIcon
                          size={16}
                          className="text-[#79A68F] shrink-0"
                        />
                  ...
                  <button
                    ...
                    className={`w-full mt-4 py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${
                      isUploading
                        ...
                        : "bg-[#79A68F] text-white hover:bg-[#5E8570] shadow-lg shadow-[#79A68F]/30"
                    }`}
                  >
                    {isUploading ? (
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-6 sm:p-8 border-b border-gray-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <p className="text-sm font-bold text-[#79A68F] mb-1">
          ...
          <div className="p-6 sm:p-8 bg-gray-50/50 grid grid-cols-1 sm:grid-cols-3 gap-6 border-b border-gray-100">
            <div className="flex gap-4 items-start">
              <div className="p-3 bg-white rounded-xl shadow-sm border border-gray-100">
                <Truck className="text-[#79A68F]" size={22} />
              </div>
              <div>
            ...
            <div className="flex gap-4 items-start">
              <div className="p-3 bg-white rounded-xl shadow-sm border border-gray-100">
                <Package className="text-[#79A68F]" size={22} />
              </div>
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-wide">
                  เลขพัสดุ
            ...
            <div className="flex gap-4 items-start">
              <div className="p-3 bg-white rounded-xl shadow-sm border border-gray-100">
                <CreditCard className="text-[#79A68F]" size={22} />
              </div>
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-wide">
                  วิธีชำระเงิน
            ...
            <div className="w-full md:w-80 space-y-4">
              <div className="flex justify-between text-sm text-gray-600">
                ...
                <span className="font-bold text-gray-900">
                  ฿
                  {(
                    orderInfo.total_amount - orderInfo.shipping_cost
                  ).toLocaleString()}
                </span>
              </div>
              ...
              <div className="flex justify-between text-sm text-gray-600">
                ...
                <span className="font-bold" style={{ color: BRAND.primary }}>
                  {Number(orderInfo.shipping_cost) === 0
                    ? "ฟรี"
                    : `฿${Number(orderInfo.shipping_cost).toLocaleString()}`}
                </span>
              </div>
