...
export default function ProductDetailPage() {
  ...
  // ✅ 1. เพิ่ม State ตรวจสอบ Login
  const [isLoggedIn, setIsLoggedIn] = useState(false);

  const isWishlisted = product ? wishlistItems?.some(w => w.id === product.id) : false;

  // ✅ 2. เช็ค Token ทันทีเมื่อเปิดหน้านี้
  useEffect(() => {
  ...
  useEffect(() => {
    const loadDetail = async () => {
      ...
      try {
        ...
        // ❌ แก้ไข: Backend ส่งมาแค่ data.data ไม่ใช่ data.data.product
        if (!data.success || !data.data) throw new Error("ไม่พบสินค้า");

        const p = data.data; // ✅ ดึงข้อมูลสินค้าออกมาตรงๆ
        const gallery = p.gallery || []; // ✅ ดึงแกลเลอรี่ออกมา
        ...
        const mapped = {
          id: p.id,
          name: p.name,
          category: p.category_name || "ทั่วไป", // ถ้า Backend ส่งชื่อหมวดหมู่มาด้วย จะดึงมาใช้ได้
  ...
  const handleAddToCart = async () => {
    if (!product) return;
    const success = await addToCart({ ...product, quantity, stock: product.stockQty });
    if (success) {
  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-gray-800">
        ...
        <button onClick={() => navigate("/shop")} className="text-[#79A68F] hover:underline font-bold">
    ...
    <div className="min-h-screen bg-white pt-24 pb-16 font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-4 mb-6">
          <button onClick={() => navigate(-1)} className="flex items-center gap-1 text-gray-500 hover:text-[#79A68F] transition font-semibold text-sm bg-gray-50 hover:bg-[#79A68F]/10 px-3 py-1.5 rounded-lg">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
          ...
          <div className="flex flex-col h-full">
            ...
            {isLoggedIn ? (
              <div className="flex flex-col sm:flex-row gap-4 mb-8">
                <div className="flex items-center justify-between bg-gray-50 p-1.5 rounded-2xl border border-gray-200 sm:w-36 shrink-0 h-14">
                  <button 
                    ...
                    className="w-10 h-10 rounded-xl bg-white shadow-sm text-gray-600 hover:text-[#79A68F] disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                  ...
                  <button 
                    onClick={increaseQty} 
                    disabled={quantity >= product.stockQty || !product.inStock} 
                    className="w-10 h-10 rounded-xl bg-white shadow-sm text-gray-600 hover:text-[#79A68F] disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                ...
                <button 
                  ...
                  className={`flex-1 flex items-center justify-center gap-3 h-14 rounded-2xl font-bold text-lg transition-all ${
                    product.inStock 
                      ? "bg-[#79A68F] text-white hover:bg-[#5E8570] shadow-lg shadow-[#79A68F]/30 hover:-translate-y-0.5" 
                ...
                <button 
                  ...
                  className={`w-14 h-14 shrink-0 flex items-center justify-center rounded-2xl border-2 transition-all ${
                    isWishlisted 
              ...
              /* ❌ 4. แสดงกล่องให้ Login แทนถ้ายังไม่มี Token */
              <div className="mt-auto bg-gray-50 rounded-2xl border border-gray-200 p-6 text-center mb-8">
                <p className="text-gray-600 mb-4 text-sm font-medium">
                  กรุณาเข้าสู่ระบบเพื่อสั่งซื้อสินค้าและเก็บเข้ารายการโปรด
                </p>
                <button 
                  onClick={() => navigate('/login')}
                  className="w-full rounded-xl bg-gray-900 hover:bg-gray-800 text-white py-3.5 font-bold transition-colors"
                >
